...
void displays::display_mouse_arrive(bool *array_seen, int n, int mx, int my, int square, int d0)
{
    if (*(array_seen + my * n + mx) == 0)
    {
        ...
        rectangle((stx + mx * square + d0 * 2), (sty + my * square + d0 * 2),
                  (stx + (mx + 1) * square - d0 * 2), (sty + (my + 1) * square - d0 * 2));
        setfillcolor(RGB(226, 255, 188));
