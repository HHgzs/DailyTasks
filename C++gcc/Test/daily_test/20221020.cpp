...
**游戏名称：OC贪吃�?/OCSnake
**游戏版本�?1.0
**游戏规则：WSAD控制光标，空格暂停，ENTER确定，ESC退�?
*/
//+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
#include <iostream>
#include <string>
...
private:
	...
	char speed;			   //蛇的速度
	const char maxRow;	   //地图最大行�?
	const char maxCol;	   //地图最大列�?
	const char initLength; //初始蛇长
	char mode;			   //游戏模式�?0-正常模式�?1-无障碍模�?
	//打印
	void print() const
	{
		int i, j;
	...
	void score()
	{
		...
		for (count = 0; count < maxCol - 6; count++)
			cout << ' ';
		cout << "你的最终得分为�?" << _SCORE(length - initLength);
		for (count = 0; count < maxRow; count++)
		{
			map[count] = new signed char[maxCol];
			memset(map[count], 0, maxCol);
